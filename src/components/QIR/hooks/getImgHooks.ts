...
import { GetImgByBatch, GetImgByItem, GetProduceImgByBarchOrDJ } from "../../../tools/api/fileApi"
...
function _getImageHooks() {
  ...
  // 获取 质量标准 投诉案例图片
  const _getImgByItem = (item:string)=>{
    _resetImg1()
    GetImgByItem(item).then((res:any)=>{
      // console.log(res)
      // _resetImg()
      if (res.length > 0) {
  ...
  return {
    _qualityStandardList, _complainCaseList, _produceImgList, _lineImgList, _getImgByItem, _getAllImgByBatch, _getImgByDECA, _resetImg1
  }
}
