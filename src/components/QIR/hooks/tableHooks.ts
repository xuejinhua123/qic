...
function _tableHooks(tableName:Ref<string>, InditexValue:Ref<string>) {
  ...
  const _resetTableData = () => {
    ...
    if(tableName.value.indexOf('Thermal') !== -1) { // target
      if (qicStore.queryForm.rbo.toUpperCase().indexOf('TARG') === -1) {
        _tableData.value.get(tableName.value)![9].results = ''
        _tableData.value.get(tableName.value)![9].isDisabled = true
      }
    }
  }
  ...
  const _initOneTable = () => {
    // 1、获取表格内容
    if (!_isTableData(tableName.value)) { // 判断是否有表格数据
      showLoading()
      GetTableJson(tableName.value).then((res:any)=>{
        _tableData.value.set(tableName.value, res.data)
  ...
  const _getOneTableContent = () => {
    ...
    // 赋值单个表格内容
    _tableContent.value = _tableData.value.get(tableName.value) || []
    // console.table(_tableContent.value)

    // 获取工序
    TABLE_SET.forEach((v: any) => {
      if (v._name === tableName.value) {
  ...
  const _filterTable = (_name:string='') => {
    ...
    // 3、过滤
    _tableProcedure.value?.forEach((v: any) => {
      if (v._title === _name) {
    ...
    // 4、targe 监听器那个也不要关
    if (_name === '印刷' || _name === '') {
      if(tableName.value.indexOf('Thermal') !== -1) {
        if (qicStore.queryForm.rbo.toUpperCase().indexOf('TARG') === -1) {
          _tableContent.value![9].results = ''
          _tableContent.value![9].isDisabled = true
        }
  ...
  const _renderingTable = (record:IQICRecord)=>{
    ...
    // (9) 如果是定时器的 被重置了，再设置一次
    // if (QTYInterval !== null && +qicStore.queryForm.qty > 7500 && qicStore.isCurrentDayRecord === 2) {
    //   console.log('渲染 定时器')
    //   middleCheck(true)
    // }
  ...
  watch(()=>qicStore.queryForm.rbo, (newVal, oldVal)=>{
    ...
    console.log(newVal, oldVal, tableName.value.indexOf('Thermal')!== -1, qicStore.queryForm.procedure, newVal.toUpperCase().indexOf('TARG') === -1)
    if (newVal !== oldVal && newVal !== '') {
      if (tableName.value.indexOf('Thermal') !== -1 && qicStore.queryForm.procedure === '印刷') {
  ...
  watch(()=>[qicStore.isCurrentDayRecord, qicStore.queryForm.qty], ([crNewVal, qtyNewVal],[crOldVal, qtyOldVal])=>{
    ...
    if(crNewVal === 2 && qicStore.queryForm.procedure === '印刷' && qicStore.isForemanLogin === false) {
