...
import { downloadPDF } from '../../tools/utils/exportPdf'
...
let _batch = ref<string>('')
const _pdf = ref<any>(null)
const handleExport = () =>{
  console.log('点击下载')
  downloadPDF(_pdf.value, document.getElementsByClassName('item'), _batch + '_' + getTime())
}
...
const mangerRecord = ()=>{
  ...
  props._tableList?.forEach((v:IQICRecord)=>{
    ...
    // 添加表工序内容
    _processAll.value?.forEach((v1:any)=>{
      if (v1._title === v._process) {
        item._table = props._tableContent.slice(v1.start - 1, v1.end)
      }
    })
