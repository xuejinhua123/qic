...
// import { GetBacthByIp } from '../../utils/api/ip_machine'
// import { GetNameByNo } from '../../utils/api/user'
// import { GetOrderByBatch } from '../../utils/api/qic_data'
// element-plus
...
// 接口
import type { IQICOrderDByIP, IQICRecord } from '../../tools/interface/iQicData'
...
// hooks
const {tableName, _tableData, _getTContent, _initOneTable, _resetContent, _disabledOneContent, _filterTableContent, _tableContent, _renderingTable } = tableHooks(ref<string>('Thermal'), InditexValue)
const { _init, _fiveData } = _useInitQICHooks()
const { _filterSearchRecordList, _autoLoader, _batchLoader, _searchRecord, _getOrderByBatch } = _useOrderHooks(isShowQIR, _orderInfo, _record)
...
const changeResults = (index: number,value: string) => {
  ...
  // 否则
    ...
    if (tbUuid.value === '') {
      ...
      _createOK(index, value, tableName.value, InditexValue.value, _tableContent.value, _filterTableContent.value)
...
const showResultsBox = (index: number) => {
  ElMessageBox.alert('检查结果不能选择NOT OK', '警告', {
    ...
    callback: () => {
      ...
      _tableData.value.get(tableName.value)![index].results = okData.value.get((index).toString()) || '' // 无效（有点问题，不见之前的值）
    },
  })
}
...
// 监听 是否是当天 (用这个监听自动加载和按单加载，用record有问题)
...
//   console.log('重置表格数据')
//   _tableData.value.get(tableName.value)!.forEach((item: any, index: number) => {
//     item.isDisabled = false
//     item.results = ''
//     item.index = index
//   })
...
// 判断是否输入了工号
// const isInputJobNumber = () => {
//   const employee:number = qicStore.queryForm.jobNumber
//   if (employee === undefined || employee === 80) {
//     messageError('请输入工号')
//     return false
//   }
//   return true
...
// const isTableData = () => {
//   // 1、判断单号
//   if(qicStore.queryForm.batch === '' || qicStore.queryForm.batch === undefined) {
//     if (batch.value === ''|| qicStore.queryForm.batch === undefined) {
//       messageError('请先选择单号')
//       return false
//     }
//   }
...
// *****************************  函  数  ********************************
// 根据单号更新单号、DJ、产品
// watch(() =>qicStore.orderData.batch, (newVal, oldVal)=>{
//   if(newVal !== oldVal) {
//     qirFrom.value.batch = qicStore.orderData.batch
//     // qirFrom.value.dj = qirStore.orderData.otc
//     qirFrom.value.internalItem = qicStore.orderData.internalItem
//   }
// })

// 根据ip更新组别和机台
// watch(() =>qicStore.ipData.ip, (newVal, oldVal)=>{
//   if(newVal !== oldVal) {
//     qirFrom.value.type = qicStore.ipData.type // 组别
//     qirFrom.value.machine_no = qicStore.ipData.machine_no // 机台号
//   }
// })
...
<template>
  <!-- 标题 -->
  <div class="qir-title">质量检查表</div>
  <!-- 搜索框 -->
  <div class="qir-search-box">
    <el-form :inline="true" :model="qicStore.queryForm" class="demo-form-inline" style="background-color: #fff;">
      ...
      <el-form-item>
        <el-button round type="primary" @click="searchQIRC" :icon="Search" :disabled="qicStore.isDisabledLeft === 1">搜索记录</el-button>
      </el-form-item>
  ...
  <!-- 表格 -->
  <div class="qir-table-content scoll" v-show="isShowQIR">
    <!-- Indeitex 表头 -->
    <div v-show="tableName === 'Inditex'" class="Inditex-header">
    ...
    <el-table
      ...
      <el-table-column prop="inspection_items" label="检查项目" width="200" header-align="center" />
  ...
  <!-- 搜索列表 -->
  <div class="qir-search-list" v-show="!isShowQIR">
    <div class="qir-search-list-item qsli_title">
      <div>工序</div>
      <div>编号</div>
      <div>姓名</div>
      <div>工号</div>
    ...
    <div class="qir-search-list-item sb_hand" :class="{active: v.state === '0'}" @click="itemQIC(v)" v-for="(v, i) in _filterSearchRecordList" :key="v.uuid">
      <div>{{v._process}}</div>
      <div>{{i + 1}}</div>
      <div>{{v.cN_Name}}</div>
      <div>{{v.employee}}</div>
    ...
    <!-- 添加一项 -->
    <div class="qir-search-list-item sb_hand" v-show="!isShowQIR">
...
.qir-search-list {
  ...
  .qir-search-list-item.active {
    background-color: #f00;
    color: #fff;
  }
  .qir-search-list-item:nth-child(1) .el-text:first-child {
    color: #f00;
  }
  .qir-search-list-item:nth-child(2) .el-text:first-child {
    color: #0f0;
  }
  .qir-search-list-item:nth-child(3) .el-text:first-child {
    color: #00f;
  }
}
