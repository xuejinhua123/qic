...
onMounted(()=>{
  ...
  condition.value = ''
  _resetImg1()
  _getImgByItem(qicStore.queryForm.internalItem)
  getSpecial()
})
...
<template>
  <div>
    ...
    <div class="qir-title qicm-title">
      <div><div>质量标准</div><div @click="()=>{_getImgByItem(qicStore.queryForm.internalItem);isReset = -1}" class="sb_hand">刷新</div></div>
    ...
    <!-- 投诉案例 -->
    <div class="qir-title qicm-title">
      <div><div>投诉案例</div><div @click="()=>{_getImgByItem(qicStore.queryForm.internalItem);isReset = -1}" class="sb_hand">刷新</div></div>
